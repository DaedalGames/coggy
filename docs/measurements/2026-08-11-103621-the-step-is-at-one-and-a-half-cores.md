...
> **A structural argument that does not need the correlation at all.** `doctor` read the clock at three load levels the same day: **94% at 1.30 cores busy, 112% at 1.60, and 199% at 14.26**. So `_Total` clock does climb strongly with load — the direction the hypothesis wants — but look at *where*. The rate steps 33% between 1.36 and 1.46 cores, across which the clock moves about 94→112%. Then from 1.5 to 13 cores the rate is **flat**, and the clock climbs 112→199% — a 78% change buying nothing. **The two are anti-aligned in where they change**, and a mechanism has to change where the effect changes. Three point samples taken minutes apart on a box whose tenancy moved between them, so the levels are approximate; the ordering is not.
