...
- **Whether `global_cpu_usage()` itself is right on this box.** Both figures here descend from it. An independent check needs a counter outside `sysinfo` — `\Processor Information(_Total)\% Processor Time` — and none was taken.
