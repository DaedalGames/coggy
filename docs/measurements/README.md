...
Seven findings, on 16 cores / 31 GiB / Windows 11, ordered by what they decide rather than when they arrived. Each links to the record that owns it; nothing here is stated that is not measured there.
...
5. **The engine figures are single readings, and the synthetic ones are not.** Every number in finding 1 comes from one `observe` run — no repeats, no fitted slope, no drift control. The synthetic redlines had to clear all three before they could be quoted, and closing [G0](../../ROADMAP.md#current-priority-m0--attribution) against an engine session means ramping eleven cooking sessions on a machine with the memory free to hold them.
...
7. **A ramp changes the machine it measures.** A rung ran 4.8% slower at the end of one ladder than at its start, dragging the fitted redline down 3.9%. Every ramp now repeats a rung as a control, and [the rule that follows](../../CLAUDE.md) is to do nothing else on the box while one runs.
