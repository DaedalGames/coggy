...
1. **An engine session costs what it costs only while cooking, and that is the state to govern.** [At rest it holds 8.97 MiB](2026-07-31-035111-between-builds.md), less than the synthetic workload that stood in for it. [While building it takes 3.27 GiB, but only one session per engine installation can build at a time](2026-07-31-034150-unreal-builds-serialise.md), so scheduling cannot change that number. [Cooking is both heavy and concurrent](2026-07-31-040348-cooking-is-the-governed-state.md) — 5.02 GiB at peak, two editors side by side, and a hundred of them wanting 193 GiB against a 22 GiB budget. **The redline for an engine session is about eleven, and cooking is what sets it** — [a Defender exclusion does not move it](2026-07-31-041923-what-an-exclusion-buys-a-cook.md), because the ceiling is memory and an exclusion returns CPU.
