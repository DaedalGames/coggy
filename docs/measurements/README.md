...
1. **What limits a hundred engine sessions is the build queue, not the machine.** [Unreal serialises build actions per installation](2026-07-31-034150-unreal-builds-serialise.md), so ten sessions ran one compiler between them. [Between builds a session holds 8.97 MiB](2026-07-31-035111-between-builds.md) — less than the synthetic workload that stood in for it — and a hundred at rest come to 897 MiB against a 22 GiB budget. The constraint sits inside a governor's reach and outside a redline's.
