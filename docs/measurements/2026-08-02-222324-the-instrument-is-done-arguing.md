...
- **[Total throughput instead of a slowdown](2026-08-02-121359-eta-follows-the-awake-count.md)** — kept for the questions it answers. A hundred-session reference reproduces to 0.94% against a solo's 4–8%, so a comparison between two configurations is quieter through it — **though 0.94% is the best of that run rather than its figure.** It is what the first three references of the no-knee sweep agreed to; the last two drifted, and [the same record puts the whole run's reference spread at 5.34%](2026-08-02-220514-there-is-no-knee.md). A hundred sessions divide a background disturbance a hundred ways and do nothing about a machine that leaves, so the quiet figure holds only while the machine holds. It gives ratios only, which is enough for *are these two the same* and not for *what is this one*.
