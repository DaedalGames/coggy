...
**[measured]** **Twelve, and it now has an error bar.** Four readings of a cooking session give [1.865 GiB ± 6%](measurements/2026-07-31-045604-an-error-bar-for-the-engine.md), which puts the ceiling between 11.4 and 12.1 sessions where a single reading had said eleven. It is still an observation rather than a redline: no fitted slope, no drift control. Closing [G0](../ROADMAP.md#current-priority-m0--attribution) against an engine session means ramping twelve cooking sessions, and twelve want 22 GiB of a machine that has had under 11 free all evening. That ceiling is neither transient nor imposed by the engine, which is what makes it the one to build admission control against. Builds are Epic's problem and Epic ships the answer as Unreal Build Accelerator — [something to consume](../CLAUDE.md) rather than schedule around.
