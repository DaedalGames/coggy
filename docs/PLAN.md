...
**Cooking is where that leaves the governor a job.** [It is heavy and, unlike a build, concurrent](measurements/2026-07-31-040348-cooking-is-the-governed-state.md) — 5.02 GiB at peak with two editors running side by side, so a hundred cooking sessions want 193 GiB against a 22 GiB budget and **the engine redline lands near eleven**. That ceiling is neither transient nor imposed by the engine, which is what makes it the one to build admission control against. Builds are Epic's problem and Epic ships the answer as Unreal Build Accelerator — [something to consume](../CLAUDE.md) rather than schedule around.
