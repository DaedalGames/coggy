...
**[measured]** Which of the residency costs actually dominates was left to `sessionbench` to decide, with [Architecture](#architecture) to be rewritten before any daemon work if the answer landed outside the list. **It did land outside** — none of the four dominates, and what binds is sessions competing for cores.

It did not land outside the architecture. The governor below already carries a core budget beside its memory quota, so what the answer changes is which of the two is load-bearing: **the core budget is, and the memory quota is a guard rail rather than the mechanism.** Admission has a form to be built against — `N ≤ 2ηC/d`, from [the duty relation](measurements/2026-07-30-154348-duty-is-derivable.md) — where `d` is the share of wall time a session computes, `C` is the machine's logical processors, and `η` is what sessions cost each other through the memory system. Both `d` and `η` are measurable per workload, neither is a constant of the machine alone, and the relation has been checked against a rung predicted before it was run.
