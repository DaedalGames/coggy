...
**Cooking is where that leaves the governor a job.** [It is heavy and, unlike a build, concurrent](measurements/2026-07-31-040348-cooking-is-the-governed-state.md) — 5.02 GiB at peak with two editors running side by side, so a hundred cooking sessions want 193 GiB against a 22 GiB budget and **the engine redline lands near eleven**. [Their peaks scatter rather than align](measurements/2026-07-31-043156-cook-peaks-scatter.md), which is what keeps that figure at eleven rather than six and makes admission a matter of counting sessions rather than scheduling them apart. The figure is [the editor loading itself rather than the project's content](measurements/2026-07-31-043951-the-editor-is-the-cost.md) — ten times the assets moved it 1.5% — so eleven holds for any project small enough that the editor dominates, and where that stops being true is the largest open assumption behind these numbers.

**[assumed]** **The eleven is a single reading, not a redline.** Every engine figure here comes from one `observe` run: no repeats, no fitted slope, and no drift control — none of [what the metric's own error bar demands](measurements/2026-07-30-164912-redline-reproducibility.md), which a synthetic redline had to satisfy before it could be quoted. Closing [G0](../ROADMAP.md#current-priority-m0--attribution) against an engine session means ramping eleven cooking sessions, and eleven of them want 21 GiB of a machine that has had under 11 free all evening. That ceiling is neither transient nor imposed by the engine, which is what makes it the one to build admission control against. Builds are Epic's problem and Epic ships the answer as Unreal Build Accelerator — [something to consume](../CLAUDE.md) rather than schedule around.
