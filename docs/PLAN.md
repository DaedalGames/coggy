...
**Three of these four have now been measured, and against synthetic sessions none of them is the bottleneck.** Processes cost little in memory though the count itself matters, [Defender's cost was overstated by two orders of magnitude](measurements/2026-07-30-142218-defender-at-scale.md) and is small, and output has three to four orders of magnitude of headroom for anything that emits text. What limited those sessions was plainer than any of them: competition for cores, at a redline of [`2ηC/d`](measurements/2026-07-30-154348-duty-is-derivable.md) — inversely proportional to the share of its time a session spends computing, and proportional to the machine's logical processors, which is the part that lets it speak for a machine other than this one.
...
**Cooking is where that leaves the governor a job.** [It is heavy and, unlike a build, concurrent](measurements/2026-07-31-040348-cooking-is-the-governed-state.md) — 5.02 GiB at peak with two editors running side by side, so a hundred cooking sessions want 186 GiB against a 22 GiB budget and **the engine ceiling lands near twelve**. [Their peaks scatter rather than align](measurements/2026-07-31-043156-cook-peaks-scatter.md), which is what keeps that figure at eleven rather than six and makes admission a matter of counting sessions rather than scheduling them apart. The figure is [the editor loading itself rather than the project's content](measurements/2026-07-31-043951-the-editor-is-the-cost.md) — ten times the assets moved it 1.5% — so eleven holds for any project small enough that the editor dominates, and where that stops being true is the largest open assumption behind these numbers.

**[measured]** **The agent CLI is a resident cost of its own, and nothing had measured it.** [580 MiB held for a session's whole life and 0.112 cores averaged over a day](measurements/2026-07-31-050322-the-agent-side-of-a-session.md), so a hundred sessions owe 58 GiB against a 22 GiB budget before an engine opens — **memory breaks at about 38 on the agent alone**. Cores stay behind it, though not by as much as that duty suggests: 0.112 belongs to a session waiting on a human, and [a driven one measures 0.27 by two independent routes](measurements/2026-07-31-054657-the-driven-duty.md), putting its core ceiling near a hundred rather than two hundred. The same process has committed 1,096 MiB against those 580 resident, and the RSS condition reads the resident figure, so every ceiling here is the optimistic one — **and the figure carries a machine state with it**: [≈97 on a rested box at every session weight from 20 to 36 MiB, and 51 on one that has recently been saturated](measurements/2026-08-03-024222-the-footprint-never-mattered.md). The 93 quoted earlier is the same formula with an `η` taken before one run's lost occupancy was separated out. The engine's figure below is paid on top of that and only while it works.

**[measured]** **Nine, once both halves of a session are on the budget.** Five readings of a cooking session give [1.87 GiB ± 6.4%](measurements/2026-07-31-045604-an-error-bar-for-the-engine.md) — twelve sessions on the engine alone — and the last of them carries a drift control that held at +0.8%. Adding the 0.52 GiB of agent CLI that drives it brings the pair to 2.39 GiB and [the ceiling to between 8.8 and 9.7](measurements/2026-07-31-054657-the-driven-duty.md). Each half was measured carefully and the sum went untaken, which is the same gap one level up from the one the agent record found. There is no slope to fit when total memory is the per-session figure times the count, so repeats and that control are [what rigour means for this condition](../ROADMAP.md#current-priority-m0--attribution) — and both are in hand without the 22 GiB a ramp would have wanted. That ceiling is neither transient nor imposed by the engine, which is what makes it the one to build admission control against. Builds are Epic's problem and Epic ships the answer as Unreal Build Accelerator — [something to consume](../CLAUDE.md) rather than schedule around.

**So demand is a hundred and capacity is nine, and that sentence had never been written down.** Every paragraph above concedes part of it — a hundred at rest, twelve cooking, thirty-eight on the agent alone — and none of them put the two numbers side by side. [G0 froze the capacity figure](measurements/2026-07-31-150258-g0-frozen.md), so the gap is **11×** and it is a property of this machine rather than of the design.

Three things close it and only one is COGGY's: **more memory** — a hundred sessions want 239 GiB, which is a machine that can be bought rather than a problem to be solved; **fewer at once for longer**, trading wall-clock for concurrency, which is the harness's call; or **more machines**, which nothing here is built for. What COGGY owns is that the daemon must not be the reason the number is lower, and admission must refuse the tenth session rather than let the machine swap.

This does not move [the two-ceiling framing below](#honest-constraint-unlocking-sessions-leaves-credits-locked) — it puts a measured figure on the infrastructure ceiling for the first time, and a target machine specification is now a thing this project has, rather than an assumption inside the number 100.
...
**[measured]** Which of the residency costs actually dominates was left to `sessionbench` to decide, with [Architecture](#architecture) to be rewritten before any daemon work if the answer landed outside the list. **It landed outside twice.** Against synthetic sessions none of the four dominates and cores bind; against an engine the first of them comes back, and memory binds before cores do.

It did not land outside the architecture. The governor below already carries a core budget beside its memory quota, and **both turn out to be load-bearing**: a synthetic session runs out of cores first, an [Unreal one runs out of memory](measurements/2026-07-31-022200-an-unreal-session.md) at thirteen sessions against twenty. Which binds is a property of the workload, so a governor that only watched one would be blind exactly when the sessions became real. Admission has a form to be built against — `N ≤ 2ηC/d`, from [the duty relation](measurements/2026-07-30-154348-duty-is-derivable.md) — where `d` is the share of wall time a session computes, `C` is the machine's logical processors, and `η` is what sessions cost each other through the memory system. Both `d` and `η` are measurable per workload, neither is a constant of the machine alone, and the relation has been checked against a rung predicted before it was run — then [again at a duty 2.8× below anything it was built on, where it holds to 9.9% and reads low](measurements/2026-08-01-080158-the-relation-at-a-quarter-duty.md), which is the conservative direction for admission. **That second check is the one a governor rests on**, since building a rule from a relation and trusting it outside the range it was fitted in are different acts. **`C` is the term to distrust, for two reasons.** On a hybrid processor the cores it counts [are not equal, and here span 2.1×](measurements/2026-07-31-145412-the-cores-are-not-interchangeable.md), so a governor reading a core count off the OS is reading three tiers as one number. And **the sessions do not receive all of them**: three hundred-session holds put the job at 14.11, 15.33 and 15.27 of sixteen, so [an `η` fitted against sixteen carries whatever else was on the box](measurements/2026-08-03-023657-eta-was-two-quantities-multiplied.md) — a difference that read as an 11% property of the workload until it was divided out. **A governor should price against the cores it measures itself holding, not against the count the OS reports**, or its ceiling moves when something unrelated wakes up.
...
This is the **target state for M1–M5.** The crates today are `sessionbench`, and `coggyd` since M1 opened — and no other box becomes a crate until measurement justifies it. The workspace also carries the three workloads, which are fixtures the benchmark drives rather than boxes in this diagram, so a count of crates comes to five and a count of boxes built to two. Scaffolding seven empty crates would freeze module boundaries before a single number earned them.
