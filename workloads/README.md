...
3. **A unit is work that finished.** Printing the line is not the work, and a workload that emits its ordinal whatever happened reports a failing loop as a fast one — an Unreal build that died in sixty milliseconds counted three units a second against a real rate of one a minute. Emit after the work succeeds, and send whatever the work said about itself somewhere a reader can find it. Silencing a tool's output is how three separate defects stayed invisible for three ramps.
