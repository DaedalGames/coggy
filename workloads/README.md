...
2. **Runs identically alone and alongside ninety-nine others.** No coordination, no shared paths, no ports. **The benchmark keeps its half of this**, giving every session its own directory under `SESSIONBENCH_SCRATCH` — for a while it did not, and a workload that built a fixed-name subtree had ten sessions deleting each other's while the ramp read them as one session running fast.
3. **A unit is work that was actually done.** Printing the line is not the work, and a loop that emits its ordinal regardless reports its own degeneration as speed. Both halves of that have bitten: an Unreal build failing in sixty milliseconds counted three units a second, and an Unreal cook returning from cache counted twelve. **Emit after the work succeeds, and clear whatever the tool would otherwise skip** — for a build that is the intermediate directory, for a cook the cooked output. Send what the tool said about itself somewhere a reader can find it; silencing that is how three separate defects stayed invisible for three ramps.

   **This rule binds the workload and nothing above it.** Anything between it and the benchmark — a `cmd /c`, a shell, a launcher — writes to the same stdout, and the benchmark counts those lines too. A switch mangled by the shell once produced three error lines per failed spawn and read as a workload running at 3 units a second, across four rungs, while the job object held no process at all. No contract a workload keeps can reach that, so the ramp now [refuses to grade a rung that never held one](../sessionbench/README.md#running-it).
4. **Knows nothing about COGGY.** A workload that takes a COGGY-specific path stops being evidence, which is [the integrity rule that workloads know nothing about `coggyd`](../sessionbench/README.md#keeping-it-honest).
5. **Exits zero when it finishes, and finishes on its own.** `--duration` exists for the workloads that do not, and a run stopped that way is labelled in its report.
6. **Writes only under a directory it created inside `SESSIONBENCH_SCRATCH`,** falling back to the temporary directory when the variable is unset. Two rules in one: a fresh directory every run, because real-time scanning charges far less the second time a file is written and a workload reusing paths measures Defender's cache instead of Defender; and a directory the benchmark named, because **the benchmark is the only thing that can remove it.** Every ramp rung ends by killing its sessions, and a killed process runs no cleanup — one working ramp left 299 directories behind before this rule existed.
