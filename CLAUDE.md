...
**Whatever launches a long run has to outlive it.** A gate hour was started as an agent's background task, which carries a ten-minute ceiling; the probes and three two-minute baselines spent it, and the run was killed the second the hour began. The tell is that nothing looked wrong — a hundred sessions went down cleanly and no process survived — so the failure reads as a finished run until you notice the artifact the end writes is missing. Detach it (`Start-Process` with the output redirected) and read the log, so the thing holding it is not something with a timeout.
