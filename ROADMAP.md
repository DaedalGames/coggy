...
**Before any of it, ask whether the box is fit to measure on — a bracket's own baseline spread answers it and needs no remembered figure.** Healthy runs here spread **0.42%, 0.39% and 0.52%** across six solo holds; on 2026-08-10 the same binary and workload mostly spread 5 to 37% across about **ten hours**, on idle and crowded machines alike. **It is not a state the box sits in**: one bracket's *before* side spread **1.1%** under twelve cores of neighbour while its *after* side spread 6.4%, so reproducibility comes and goes inside four minutes and a single tight triple is not a recovery. A hold now prints the reading beside the number. **The work-rate condition is a ratio of two solo means and is unmeasurable while that state holds**, which is a fact about this hardware rather than about the daemon, and the check is what says when it ends. [The evening that mistook it for the instrument's noise floor](docs/measurements/2026-08-03-173452-the-slow-state-flatters-the-gate.md) is worth reading before deriving anything from a spread.
...
**And within that regime it is flat, so the constant survives** — which narrows the paragraph above rather than contradicting it, since a constant is a function of `N·d` that happens not to vary. Read directly rather than through a solo baseline, [`η` moves 1.10% against 0.69% of drift while the awake count rises 67%](docs/measurements/2026-08-02-194155-eta-is-flat-where-it-was-said-to-fall.md). An earlier reading had it falling — 0.733 against about 0.8 — and both halves of that comparison were weaker: each came through a solo hold, which is the loudest term in a slowdown at 4–8% against 0.94% — *not* the noisiest number this instrument makes, a superlative withdrawn once [three of that day's four solo sets came back at 3.9–8.4%](docs/measurements/2026-08-02-221853-the-noisy-baseline-was-one-noisy-afternoon.md) — and the higher one sat at `N·d ÷ C = 1.07`, a tenth above saturation where the relation's own assumption stops holding. **So `2ηC/d` is usable with a fixed `η` above saturation**, and what needs care is the boundary — which is exactly where a governor decides whether to admit one more.
