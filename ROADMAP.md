...
**What is still open is the workload**, and it is the term that decides the outcome: duty sets how many sessions are awake at once and footprint sets `η`, so *a hundred sessions* names a count and not a load. This gate is run at the measured driven duty of 0.27 for that reason, and the footprint is bounded by the RSS condition rather than chosen. Cold start under 30 seconds is a nice-to-have recorded in the startup log, not a gate — [it is 0.02% of the workload](docs/PLAN.md#residency-not-spawning).
