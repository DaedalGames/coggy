...
**The gate is not mis-specified, it is above this machine.** The condition needs `N·d ÷ (η·C) ≤ 2`, so at a hundred sessions and the measured duty it wants **about nineteen logical processors** where this box has sixteen — a floor, since `η` on a wider machine is unmeasured and more cores can mean more claimants on one memory system. [The steps, for whoever runs it on a machine wide enough](sessionbench/README.md#running-gate-m1-on-a-machine-that-can-pass-it) — including which figures to re-measure there rather than carry from here. The same run also moves the core ceiling from 93 to **87** — and the two are one formula with two inputs, since `2ηC/d` gives 93 at `η = 0.785` and 87 at the 0.733 this run implies. **The ceilings are the `η` disagreement written another way**, not a measurement checking a projection. Both sit far above [the nine a real generation session allows](docs/measurements/2026-07-31-150258-g0-frozen.md), so neither moves G0, and [the value itself rests on a single slowdown](docs/measurements/2026-08-01-213912-the-gate-breaks-at-its-own-duty.md) until the knee is run.
