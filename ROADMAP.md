...
**And the memory the gate leaves spare does not buy enough work rate to matter.** At 33 MiB a session, the most the budget allows, a hundred sessions slow down **3.261×** while RSS climbs to 3.648 GiB of 3.725 — so the gate fails at both weights anyone has run. `η` does rise with the weight, by about a fifth: [compared inside one machine state, 20 MiB gives 3.958 and 33 MiB 3.261](docs/measurements/2026-08-03-003443-the-footprint-result-was-the-machine.md). It is nowhere near the 2 the condition asks for.
...
**So the bottleneck is a decision, not an implementation.** Either the gate says what it asks of the hardware it runs on, or it runs on hardware that can answer it — roughly 15% more cores than this box has, and one that survives an hour of saturation. Writing more daemon does not move either number, and neither does choosing a different session weight — **and any figure quoted for this gate has to name the machine state it was taken in**.
...
**And whatever shape it takes, it is per session weight — and by less than the machine's own state.** [Compared inside one state, a hundred sessions at 20 MiB slow down 3.958× and at 33 MiB 3.261×](docs/measurements/2026-08-03-003443-the-footprint-result-was-the-machine.md), moving the implied admission ceiling from 51 sessions to 61 — **21% from a 13 MiB change in what each one holds**, with the heavier session costing its neighbours less. The same 20 MiB hold on a rested box gives 2.301× and a ceiling of 87, so **the state is worth more than the weight** and a ceiling quoted without both is not quoted. A governor reading one ceiling and applying it to whatever arrives is reading a number for a workload it may not be admitting, on a machine it may not be admitting it on. What sets the direction is unmeasured — the footprint does not change what a unit costs alone, so it is contention of some kind and a slowdown does not say which.
