...
**1.69× is the floor rather than the estimate**, since it assumes sessions cost each other nothing. [A run at a hundred sessions](docs/measurements/2026-08-01-202112-gate-m1-at-twenty-minutes.md) puts that term at about 0.855, which carries the prediction at the intended duty to **1.98× — inside 2 by a percent.** So the condition is expected to hold and expected to be close, and **stating the duty is not enough: a run has to measure the duty it actually held.** A fixed wait calibrated on a warm machine gave 0.172 on a cold one, because it turns a 79% difference in compute speed into 13.6% of work rate.
...
**One of these three cannot be asked of a daemon, and that is a property of the arrangement rather than of the run.** Dropped output is found by watching ordinals in a session's own stream; under `coggyd` that stream ends in the daemon's scrollback, so nothing here holds what the workload emitted and there is nothing to subtract from. It is reported `out_of_reach`, a word kept distinct from `not_taken`: the work-rate condition needed a second run rather than a different daemon, and [`hold --with-solo` is that run](sessionbench/README.md#running-it) — a solo baseline on either side of the concurrent hold, refusing to produce a ratio when the two disagree by more than the machine makes while being itself.
