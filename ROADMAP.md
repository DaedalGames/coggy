...
**Seven instrument commits landed after those holds and none of them can move a gate figure**, which is checked rather than asserted: no line in any of them touches how a rate, an RSS peak or a verdict is computed. They add fields and printing. The one that runs on every tick — a per-core CPU counter read — costs **0.2 ms**, measured at a matched pair: tick slip of 23.7 and 23.8 ms at a hundred sessions on a 5 s tick before it existed and 23.9 after, with the median at exactly 24.0 in all three.
