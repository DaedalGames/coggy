...
**Before spending an hour, ask the box whether it can supply a baseline at all — [`scripts/wait-for-quiet.ps1`](scripts/wait-for-quiet.ps1) answers that in twenty minutes of light load.** The work-rate condition is a ratio whose numerator must come from a *rested* box: the published figures are clean because their six solo holds read 21.66–21.90 at 0.42% spread, and a hold in the fourteens would have been a tenanted reading. The script waits for a quiet window, takes one 30-second solo hold, reads its rate, and keeps waiting unless it clears `RestedAbove` (18.0, below the rested band's floor of 18.9 and above every tenanted reading recorded). **Exit 0 means a rested window exists and the gate can be armed now; exit 4 at `MaxHolds` means the day is unusable and you found out for twenty minutes rather than an hour.** Launch it detached — it writes its own timestamped transcript under `bench-out/` so no run overwrites another's.

It answers a real question rather than a hypothetical one: on 2026-08-11 this box [topped out at 17.222 across thirteen holds](../docs/measurements/2026-08-11-103621-the-step-is-at-one-and-a-half-cores.md), about 9% below the band floor, so no gate run that day could have produced a usable numerator however long it waited.
...
2. **If you must size before you buy: this box needs about 3% more cores.** Its slowdown is **2.0654**, and `C_required = C × slowdown ÷ 2` — which is what `N·d ÷ 2η` reduces to once `η` is the `N·d ÷ (C·s)` this machine's own run gave. So "16.52 logical processors" is *sixteen times 2.0654 over two*, and it holds only if the wider machine has the same `η`. It is a starting estimate wearing the clothes of a requirement.

   **The 2.301 this said until now was one disturbed run**, and the correction is worth more than the three percent: that hold [lost the machine to something else in 18% of its samples](../docs/measurements/2026-08-03-024222-the-footprint-never-mattered.md), which a mean absorbed and a buyer would have paid for. Sizing off a single slowdown means sizing off whatever else was running that afternoon.
3. **Check the plug.** [The same command returns 7.8× less on battery](../docs/measurements/2026-08-02-195840-the-same-command-on-battery.md) with every other recorded figure identical, so a laptop measured unplugged is a different machine. `doctor` says which state it is in. **And nothing else will tell you** — not even the baseline check in step 6: [two quiet holds ninety minutes apart agreed to 2.6% across the boundary](../docs/measurements/2026-08-11-163342-a-solo-baseline-cannot-see-the-plug.md), because the power ceiling binds under saturation and a lone session at duty 0.27 asks for a quarter of one core out of sixteen. A solo rung agreeing proves the box is not in the slow state and says nothing about the plug. `compare` refuses a pair that crosses one; a single run has only `doctor` and the `on_battery` field every artifact carries.
4. **The hour needs a rested box and then unmakes one, so read the bracket's two sides against the bands rather than against each other.** A hundred saturating sessions is what induces the slow state, at somewhere between twenty and forty-one minutes — so the opening solo rungs measure a rested machine and the closing ones measure the machine this run slowed. Three things decide whether the result is about the gate: both sides' `solo rest` low, one to three cores, since a tenant on either side is invisible to an agreement check; the opening level in the rested band rather than the other two, because [a crowded rested box outruns a quiet slow one](../docs/measurements/2026-08-03-094550-the-slow-state-caught-on-a-quiet-machine.md) and a middling rate is not a middling machine; and a closing level near the slow band **with the rest figure still low** read as this run's own doing rather than as drift. On the box these were measured on that is 18.9 rested and 9.0 slow — **from two holds and twenty respectively**, so the one a gate run must land in is the one measured least. **Tenanted is a band rather than a point**: 11.95–17.10, mean 14.741, across 48 solo mains holds, where an earlier 13.8 came from seven. [The band's shape is a step at about 1.4 cores held](../docs/measurements/2026-08-11-103621-the-step-is-at-one-and-a-half-cores.md) with flat shelves either side, so a hold at 2 cores held and one at 12 read much the same and the figure says presence rather than size.

   **And a pair speaks for ten minutes, where the hour needs an hour.** Two solo holds say whether the machine was steady while they ran and nothing about where it is heading — two such pairs half an hour apart were each internally tight and sat 8.7% from one another. Before starting, read every one-session hold from the last two hours out of `bench-out/` as a series of sliding two-hold means. That is what showed this box jumping once and then holding a new level for forty minutes, and [none of those holds was run to answer it](../docs/measurements/2026-08-03-094550-the-slow-state-caught-on-a-quiet-machine.md) — the check costs a directory read, not a measurement.

5. **Run the hour only if the machine survives it, and know which load stopped it.** Sustained full-core load stopped this one at forty-one minutes with no bugcheck — on `--wait-ms`, whose duty climbs as the machine slows, so what crashed was a rising load rather than the flat 0.27 the gate is stated in. Every twenty-minute hold at a fixed duty has finished clean and none has failed — a count is not given because `bench-out/` is pruned to the newest run or two, so nobody can check one. **The hour is untried here rather than tried and failed**, and it is the run to have a human present for: a hard stop takes any session-scoped scheduler with it, and the same run is the only test of [the slow machine state's twenty-to-forty-one-minute threshold](../docs/measurements/2026-08-03-004512-a-saturating-burst-halves-the-box-for-an-hour.md).
6. **Read the drift control before the verdict, and the `solo rest` line before the drift control.** `hold --with-solo` brackets the run and refuses a ratio when its two baselines disagree by more than the machine makes while being itself. Disagreement is the symptom; `solo rest` reports the cores held outside the job while each side ran, and the two causes it separates want opposite things — a machine that drifted is worth rerunning now, a machine with twelve cores held throughout is worth waiting out. **Agreement cannot make that call**: two baselines inside one disturbance agree with each other and are both wrong.
7. **State the duty the run actually held**, not the one it asked for. `--duty` delivers a stated duty on any machine; a fixed `--wait-ms` does not, and [calibrated warm and run cold it delivered 0.172 where 0.271 was asked for](../docs/measurements/2026-08-01-210316-the-wait-mechanism-cancels.md).
...
**And a matching `os_version` no longer means a matching machine.** On 2026-08-12 this box began parking cores: six quiet hundred-session holds, the neighbour absent in all of them, gave **16.11 and 16.00 machine cores on 3 and 11 August against 4.52-5.37 on the 12th** — and every one of those artifacts records the same build, `11 (26200)`. Two Windows updates installed that day and nothing in the host block could see them. `updates` now carries the six most recent, so two runs days apart differ visibly in a field designed to differ. **Check it before comparing anything across days**, on the same reasoning as the plug: a build string is a version, not a configuration.
...
**That is a rule about committing them, not about reading them, and the difference cost a night.** One night produced six findings and five came out of columns that every run had been writing for days and nobody had opened — occupancy, the intervals a hold lost the machine, the memory that went missing alongside, the process count that ruled out sessions exiting, Defender's flat share, and the absence of a step at the scrollback cap. So prune `bench-out/` once the record exists, and **read the samples before concluding the instrument is missing something.** The reports summarise; a mean in one of them hid an interruption that was read as a workload property for three hours.
