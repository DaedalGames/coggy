...
| coggyd | Ours. **Added only after M1** | M1 — the daemon exists and [holds a hundred sessions for an hour](../docs/measurements/2026-08-01-103225-an-hour-of-a-hundred-sessions.md), but a ladder cannot be put around it yet: it starts every session from one command in one directory, and [the contract's rule 2](../workloads/README.md#the-contract) wants each its own. The ramp keeps that half itself and the daemon has no equivalent |
