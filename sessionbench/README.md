...
**Two conditions come back unanswered whatever you do, and they say so in a different word from the one that was merely waiting.** Dropped output and replacement are `out_of_reach`: every other target hands this process the reading end of each session's output, which is what finds a gap in the ordinals, and under the daemon that end belongs to the daemon — while nothing in it restarts a session that exited. More running would not help either.
...
**Two runs live in [`scripts/`](scripts/) rather than in a note**, because a measurement that waits on a window may wait across sessions and a window that opens is not a window to type in. `m1-hour.ps1` is the gate's sixty-four minutes; `bracket-calibration.ps1` is the five short holds that decide whether the bracket's allowance is asking the right question. Both read their own precondition and exit non-zero rather than producing a run that cannot mean anything, and they ask for different things. `m1-hour.ps1` wants steadiness rather than quiet — the spread of consecutive `doctor` readings, since a ratio hands both halves the same handicap and a steady 40% is workable where a swinging 34% is not. **`bracket-calibration.ps1` asked for the same and got nineteen refusals in a day**, so it now asks for nothing but room for its own load and repeats its sequence instead, taking the verdict rule from [`exclusion-delta`](#running-it): spread across repeats against the effect, inconclusive when the first is larger. A repeat measures whether the machine held still; a precondition only assumes it.
...
| coggyd | Ours. **Added only after M1** | `ramp --daemon <path>` runs — ten rungs of `ping` gave 11 processes and 62.14 MiB at ten sessions. What the row still wants is a ladder on a quiet machine. **It measures two of the four conditions**, for the reason below |
| Windows Terminal, WezTerm, Alacritty, wmux | What an emulator costs to host N sessions | a different instrument · M4 |

**Our own row will be a partial, and saying so is the point.** Every other row spawns the sessions itself and holds the reading end of each one's output, which is what lets it count units and notice a gap in them. Under `coggyd` the daemon holds that end: a session's output reaches its scrollback and stops there. Work rate survives with a different numerator — the daemon reports how many lines it read — and RSS is untouched, since the job this process creates still contains the daemon and everything under it. **Dropped output does not survive.** Gaps are found by watching ordinals in a session's stream, nobody here holds what the workload emitted, and there is nothing to subtract from. Replacement does not survive either: nothing in the daemon restarts a session that exited.
